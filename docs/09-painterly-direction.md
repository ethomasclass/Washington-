...
| # | Thing | State | Note |
|---|---|---|---|
| 1 | `src/art.ts` — 3,800 lines of procedural plates, props and figures | **Superseded, not yet replaced** | This was always scaffolding. It is flat-fill vector shapes, which `02` §9.13 already banned; it is not a casualty of the pivot so much as a debt the pivot makes due. The replacement is generated plates, not hand-written painterly drawing code |
