...
| 1a | Form lighting and air | **Done at the primitive level** | The plates now carry a key light. `setPlateLight()` points it from the scene's own sun before the plate set is built, and `solid()` lights every mass off it: shadow strokes pulled to the far side, darker **and cooler**; loaded strokes pulled to the lit side, lighter **and warmer**. Warm light against cool shadow is what makes a form read as three-dimensional at a narrow value range, and it is the half people leave out. Cambridge in July gets a weak key (`k` 0.45) because flat overcast has a direction and no shadow worth the name. Atmospheric recession is laid per plate, on the two furthest layers only |
| 1b | The *composed* value structure | **Still not done, and still the real gap** | Form lighting lights each mass on its own. It cannot give the frame a dark foreground mass, a genuine light source inside the composition, or a focal area the eye is led to — those are decisions in the scene painters, not in a primitive. This is what still separates the plates from the reference image, and it is per-scene work |
